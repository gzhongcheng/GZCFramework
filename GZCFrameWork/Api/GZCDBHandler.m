...
typedef NS_ENUM(NSInteger, GZCDBTableCmpResult) {
    GZCDBTableNotExist       = 1,
    GZCDBTableChanged        = 2,
    GZCDBTableTheSame        = 3,
    GZCDBTableMigratable     = 4,
};

@interface GZCDBHandler () {
    FMDatabaseQueue *GZCDBQueue;
}
@end

static GZCDBHandler *dbHandler = nil;
@implementation GZCDBHandler
...
+ (GZCDBHandler *)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        dbHandler = [[GZCDBHandler alloc] init];
    });
    return dbHandler;
}
...
- (instancetype)init {
    if (self = [super init]) {
        GZCDBQueue = [FMDatabaseQueue databaseQueueWithPath:[GZCDBHandler dbFilePath]];
...
/*
 ...
 [GZCDBQueue inDatabase:^(FMDatabase *db) {
 @try {
 if (![db open]) {
        ...
        if (cmpResult != GZCDBTableTheSame) {
            [GZCDBQueue inDatabase:^(FMDatabase *db) {
                @try {
                    ...
                    NSString *createSQL = [GZCDBHandler createTableSQLWithModel:(NSObject *)model inDb:db];
...
-(NSArray *)queryWithClass:(Class)modelClass key:(NSString *)key greaterValue:(NSNumber *)value orderByKey:(NSString *)oKey desc:(BOOL)desc{
    NSMutableArray * resultObjArray = [NSMutableArray array];
    NSString * tableName = NSStringFromClass(modelClass);
...
- (NSArray *)quertWithClass:(Class)modelClass
    ...
    // condition
    if (key != nil && value1 != nil && value2 != nil) {
        if([value1 isKindOfClass:[NSNumber class]]&&[value2 isKindOfClass:[NSNumber class]]){
            sqlString = [sqlString stringByAppendingString:[NSString stringWithFormat:@"where %@ between ? and ?", key]];
        }
        else
        {
            // object other than nsstring nsnumber is not supported for now
    ...
    [GZCDBQueue inDatabase:^(FMDatabase *db) {
        @try {
            ...
            FMResultSet* result = [db executeQuery:sqlString,value1,value2];
            //FMResultSet* result = [db executeQuery:@"SELECT * FROM TZSUser where usrId LIKE 'r817k5d6'"];
            while ([result next]) {
                id<NSObject> obj = [self objectFromFMResult:result byClass:modelClass];
                if (obj != nil) {
...
-(NSArray *)queryWithClass:(Class)modelClass key:(NSString *)key lessThanValue:(NSNumber *)value orderByKey:(NSString *)oKey desc:(BOOL)desc{
    NSMutableArray * resultObjArray = [NSMutableArray array];
    NSString * tableName = NSStringFromClass(modelClass);
...
- (GZCDBTableCmpResult) verifyCompatibilyForTable: (NSObject *) model
{
    if (![self isTableExist:NSStringFromClass(model.class)]) {
        return GZCDBTableNotExist;
    }
    
    // try the migration version first
    if ([model.class conformsToProtocol:@protocol(GZCDBMigrationProtocol)]) {
    ...
    if (currentTableColNo != storePropertyCount) {
        return GZCDBTableChanged;
    }
    return  GZCDBTableTheSame;
}
...
{
    NSString * tableName = NSStringFromClass([GZCDBVersionTable class]);
    if (![self isTableExist:tableName]) {
        [GZCDBQueue inDatabase:^(FMDatabase *db) {
            @try {
                ...
                NSString *createSQL = [GZCDBHandler createTableSQLWithModel:(NSObject *)[[GZCDBVersionTable alloc]init] inDb:db];
                db.shouldCacheStatements = YES;
                if (![db executeUpdate:createSQL]) {
                    NSLog(@"create DB fail - %@", createSQL);
                };
...
{
    GZCDBVersionTable * tVersion = [[GZCDBVersionTable alloc]init];
...
{
    NSString * tableName = NSStringFromClass(modelClass);
    NSArray * tVersionArray = [self queryWithClass:[GZCDBVersionTable class] key:@"tablename" value:tableName orderByKey:nil desc:NO];
    if (tVersionArray != nil && [tVersionArray count] == 1) {
        GZCDBVersionTable * tVersion = tVersionArray.lastObject;
...
- (BOOL) migrateClassTable: (NSObject *) model
{
    ...
    GZCDBVersionTable * newDBTVersion = [[GZCDBVersionTable alloc]init];
...
- (void) dumpObject: (NSObject *) obj
{
    ...
    if ( [obj conformsToProtocol:@protocol(GZCDBMigrationProtocol)]) {
