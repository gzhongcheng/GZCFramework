...
/**
 2017.7.7 新增按钮毛玻璃效果  by  G.Z.C
 */
@property (nonatomic , assign ) LEEBackgroundStyle modelActionStyle;
@property (nonatomic , assign ) UIBlurEffectStyle modelActionBlurEffectStyle;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _modelActionStyle = LEEBackgroundStyleTranslucent; //默认为半透明背景样式
        
        _modelBackgroundBlurEffectStyle = UIBlurEffectStyleDark; //默认模糊效果类型Dark
        _modelActionBlurEffectStyle = UIBlurEffectStyleDark; //默认模糊效果类型Dark
    }
    return self;
}
...
- (LEEConfigToBlurEffectStyle)LeeActionStyleBlur{
    ...
    return ^(UIBlurEffectStyle style){
        ...
        if (weakSelf) {
            ...
            weakSelf.modelActionStyle = LEEBackgroundStyleBlur;
            
            weakSelf.modelActionBlurEffectStyle = style;
        }
...
@property (nonatomic , strong ) UIVisualEffectView *actionVisualEffectView;
...
- (void)configAlert{
    ...
    for (id item in self.config.modelActionArray) {
        ...
        if (!action.backgroundColor) action.backgroundColor = [UIColor whiteColor];
...
- (void)updateActionSheetLayoutWithViewWidth:(CGFloat)viewWidth ViewHeight:(CGFloat)viewHeight{
    ...
    self.actionVisualEffectView.frame = CGRectMake(CGRectGetMinX(self.actionSheetView.frame), CGRectGetMinY(self.actionSheetView.frame), CGRectGetWidth(self.actionSheetView.frame), CGRectGetMaxY(self.actionSheetCancelAction.frame) - CGRectGetMinX(self.actionSheetView.frame));
...
- (void)configActionSheet{
    ...
    if (self.config.modelActionStyle == LEEBackgroundStyleBlur) {
        
        UIBlurEffect *blur = [UIBlurEffect effectWithStyle:self.config.modelActionBlurEffectStyle];
        
        self.actionVisualEffectView = [[UIVisualEffectView alloc] initWithEffect:blur];
        
        self.actionVisualEffectView.alpha = 1.0f;
        
        [self.view insertSubview:self.actionVisualEffectView belowSubview:self.actionSheetView];
    }
