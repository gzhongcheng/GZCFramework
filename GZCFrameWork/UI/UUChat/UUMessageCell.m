...
- (void)btnContentClick{
    ...
    }else if(self.messageFrame.message.type == UUMessageTypeLink){
        if ([self.delegate respondsToSelector:@selector(cellLinkDidClick:link:)])  {
            [self.delegate cellLinkDidClick:self link:self.btnContent.link];
        }
    }
}
...
- (void)setMessageFrame:(UUMessageFrame *)messageFrame{
    ...
    self.btnContent.linkLabel.hidden = YES;
    self.btnContent.linkImageView.hidden = YES;
    ...
    if (message.from == UUMessageFromMe) {
        ...
        self.btnContent.linkImageView.frame = CGRectMake(10, 10, 80, 80);
        self.btnContent.linkLabel.frame = CGRectMake(MaxX(self.btnContent.linkImageView)+5, MinY(self.btnContent.linkImageView), 150, 80);
        self.btnContent.linkLabel.textColor = [UIColor whiteColor];
    }else{
        self.btnContent.isMyMessage = NO;
        [self.btnContent setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        self.btnContent.contentEdgeInsets = UIEdgeInsetsMake(ChatContentTop, ChatContentLeft, ChatContentBottom, ChatContentRight);
        self.btnContent.linkImageView.frame = CGRectMake(20, 10, 80, 80);
        self.btnContent.linkLabel.frame = CGRectMake(MaxX(self.btnContent.linkImageView)+5, MinY(self.btnContent.linkImageView), 150, 80);
        self.btnContent.linkLabel.textColor = mainFontColor;
    }
    ...
    switch (message.type) {
        ...
        case UUMessageTypePicture:
        {
            ...
            [self.btnContent.backImageView setWebImage:message.pictureUrl placeholderImage:message.picture];
        ...
        case UUMessageTypeVoice:
        {
            ...
            int time = [message.strVoiceTime intValue];
            CGRect frame = self.btnContent.frame;
            frame.origin.x += frame.size.width - 80;
            frame.size.width = 80;
            frame.size.width += time*4;
            frame.origin.x -= time*4;
            self.btnContent.frame = frame;
...
//            voiceURL = [NSString stringWithFormat:@"%@%@",RESOURCE_URL_HOST,message.strVoice];
            ...
            break;
        case UUMessageTypeLink:
        {
            self.btnContent.linkLabel.hidden = NO;
            self.btnContent.linkImageView.hidden = NO;
            self.btnContent.linkLabel.text = message.strContent;
            [self.btnContent.linkLabel sizeToFit];
            self.btnContent.linkLabel.frame = CGRectMake(MinX(_btnContent.linkLabel), MinY(_btnContent.linkLabel), WIDTH(_btnContent.linkLabel), HEIGHT(_btnContent.linkLabel)>80?80:HEIGHT(_btnContent.linkLabel));
            [self.btnContent.linkImageView setWebImage:message.pictureUrl placeholderImage:message.picture];
            self.btnContent.link = message.link;
        }
            break;
        default:
            break;
