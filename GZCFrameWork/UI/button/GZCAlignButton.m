...
-(void)setBadge:(NSString *)badge{
    ...
    if (_badge==nil || [_badge isEqualToString:@""]) {
...
- (void)layoutImageAndTitle{
    ...
    switch (_textAligment) {
        ...
        case GZCAlignButtonAlignmentTop:
        {
            ...
            badgeX = CGRectGetWidth(_imageView.frame) > 0 ? (CGRectGetMaxX(_imageView.frame) - _badgeRadius * 1):(CGRectGetMaxX(_titleLabel.frame)-_badgeRadius);
            badgeY = CGRectGetMinY(_titleLabel.frame) < CGRectGetMinY(_imageView.frame) ? CGRectGetMinY(_titleLabel.frame) : CGRectGetMinY(_imageView.frame) - _badgeRadius * 1;
            break;
        }
        case GZCAlignButtonAlignmentBottom:
        {
            ...
            badgeX = CGRectGetWidth(_titleLabel.frame) > 0 ? (CGRectGetMaxX(_titleLabel.frame)-_badgeRadius):(CGRectGetMaxX(_imageView.frame) - _badgeRadius * 2);
            badgeY = CGRectGetMinY(_imageView.frame) < CGRectGetMinY(_titleLabel.frame) ? (CGRectGetMinY(_imageView.frame)):(CGRectGetMinY(_titleLabel.frame));
            break;
        }
    ...
    if (_badgeStyle != GZCAlignButtonBadgeStyleRibbon) {
        _badgeLabel.frame = CGRectMake(badgeX, badgeY, CGRectGetWidth(_badgeLabel.bounds), CGRectGetHeight(_badgeLabel.bounds));
    }
}
...
-(void)layoutBadgeView{
    switch (_badgeStyle) {
        ...
        case GZCAlignButtonBadgeStyleRibbon:{
            _badgeLabel.text = _badge;
            [_badgeLabel sizeToFit];
            CGPoint centerPoint = CGPointMake(CGRectGetWidth(self.bounds) - CGRectGetWidth(_badgeLabel.bounds)/2 - 10, CGRectGetWidth(_badgeLabel.bounds)/2 + 10) ;
            float width = CGRectGetWidth(_badgeLabel.bounds) + CGRectGetWidth(self.bounds);
            float heigh = CGRectGetHeight(_badgeLabel.bounds);
            _badgeLabel.bounds = CGRectMake(0 , 0, width, heigh);
